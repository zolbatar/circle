...
#define HEAP_DMA30 HEAP_LOW // 30-bit DMA-able memory
    {
        ...
        {
        case HEAP_LOW: {
            auto m = s_pThis->m_HeapLow.Allocate(nSize);
            TaskAddAllocation(nSize, m);
            return m;
        }
        case HEAP_HIGH: {
            auto m = s_pThis->m_HeapHigh.Allocate(nSize);
            TaskAddAllocation(nSize, m);
            return m;
        }
        case HEAP_ANY: {
            auto m = (pBlock = s_pThis->m_HeapHigh.Allocate(nSize)) != 0 ? pBlock : s_pThis->m_HeapLow.Allocate(nSize);
            TaskAddAllocation(nSize, m);
            return m;
        }
        default:
    ...
    static void *HeapReAllocate(void *pBlock, size_t nSize) // pBlock may be 0
    {
        ...
        {
            TaskFreeAllocation(pBlock);
            auto m = s_pThis->m_HeapLow.ReAllocate(pBlock, nSize);
            TaskAddAllocation(nSize, m);
            return m;
        }
        else
        {
            TaskFreeAllocation(pBlock);
            auto m = s_pThis->m_HeapHigh.ReAllocate(pBlock, nSize);
            TaskAddAllocation(nSize, m);
            return m;
        }
#else
    ...
    size_t GetHeapFreeSpace(int nType) const
    {
        ...
        case HEAP_HIGH:
            return s_pThis->m_HeapHigh.GetFreeSpace();
        case HEAP_ANY:
            return s_pThis->m_HeapLow.GetFreeSpace() + s_pThis->m_HeapHigh.GetFreeSpace();
        default:
            return 0;
        }
#else
