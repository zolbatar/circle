...
CTranslationTable::CTranslationTable(size_t nMemSize) : m_nMemSize(nMemSize), m_pTable(0)
{
    ...
    for (unsigned nEntry = 0; nEntry < LEVEL2_TABLE_ENTRIES; nEntry++) // entries a 512MB
    {
        ...
        if (nBaseAddress >= 4 * GIGABYTE &&
            !(MEM_PCIE_RANGE_START_VIRTUAL <= nBaseAddress && nBaseAddress <= MEM_PCIE_RANGE_END_VIRTUAL))
        {
            continue;
        }
#endif
