...
#define TTBR_MODE (ARM_TTBR_INNER_CACHEABLE | ARM_TTBR_OUTER_NON_CACHEABLE)
#elif RASPPI <= 3
...
CPageTable::CPageTable(u32 nMemSize) : m_pTable((u32 *)MEM_PAGE_TABLE1)
{
...
CPageTable::CPageTable(u32 nMemSize) : m_nMemSize(nMemSize), m_pTable((u64 *)MEM_PAGE_TABLE1)
{
    memset(m_pTable, 0, PAGE_SIZE);

    for (unsigned nEntry = 0; nEntry < LEVEL1_TABLE_ENTRIES; nEntry++) // entries a 1GB
    {
        u64 nBaseAddress = (u64)nEntry * ARMV7LPAE_TABLE_ENTRIES * ARMV7LPAE_LEVEL2_BLOCK_SIZE;

        u64 *pTable = CreateLevel2Table(nBaseAddress);
        assert(pTable != 0);

        TARMV7LPAE_LEVEL1_TABLE_DESCRIPTOR *pDesc = (TARMV7LPAE_LEVEL1_TABLE_DESCRIPTOR *)&m_pTable[nEntry];
...
u64 *CPageTable::CreateLevel2Table(u64 nBaseAddress)
{
    ...
    {
        TARMV7LPAE_LEVEL2_BLOCK_DESCRIPTOR *pDesc = (TARMV7LPAE_LEVEL2_BLOCK_DESCRIPTOR *)&pTable[nEntry];
        ...
        if (nBaseAddress >= (u64)&_etext)
        {
            ...
            if ((nBaseAddress >= m_nMemSize && nBaseAddress < MEM_HIGHMEM_START) || nBaseAddress > MEM_HIGHMEM_END)
            {
                pDesc->AttrIndx = ATTRINDX_DEVICE;
                pDesc->SH = ATTRIB_SH_OUTER_SHAREABLE;
            }
            else if (nBaseAddress >= MEM_COHERENT_REGION && nBaseAddress < MEM_HEAP_START)
            {
